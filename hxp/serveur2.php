...
function get_EUBeds($args)
{
	...
	$msg="";
/*
	$requete = "SELECT Hop_finess,Hop_nom,Hop_DZ,type_etablissement_ID,ville_longitude,ville_latitude,ville_ZIP,ad_zone1
			FROM hopital, adresse, ville
			WHERE hopital.adresse_ID = adresse.ad_ID
			AND adresse.ville_ID = ville.ville_ID
			AND ville.departement_ID IN ('67')
			";
	//$resultat = ExecRequete($requete,$connexion);
	...
	//while($rub=mysql_fetch_array($resultat))
	{
		$msg .="<Hospital>";
    		$msg .="<Basic_Data>";
      			...
      			if($i==0)$msg .="<Designation>$rub[Hop_nom]</Designation>";
			$msg .="</Basic_Data>";
		$msg .="</Hospital>";
	}
	}
	...
	$msg = "<essai><Test>hopital</Test><Test>truc</Test></essai>";
*/
	return $msg;
}
