...
$type_materiel = $_REQUEST[type_materiel];
$image_width = 660;
$image_heigth = 800;
...
{
	$file = $backPathToRoot."carto/d".$rub[0].".txt";
...
$requete = "SELECT SUM(dotation_qte)AS qte, dotation.ville_ID,ville_nom,ville_lambertX,ville_lambertY
			FROM dotation, ville
...
while($rub=mysql_fetch_array($resultat))
{
	...
	/* décalage par rapport à la ville pour écrire la quantité */
	$dx=5;
	$dy=-20;
	$size = 15;
	$fonte = 'IB';
	$d->ecrire($rub['qte'],$rub[3]/1000,$rub[4]/1000,$dx,$dy,$rouge,$size,'',$fonte);
	...
	$size = 8;
	$angle=90;
	//$d->ecrire($copyright,$rub[3]/1000,$rub[4]/1000,$dx,$dy,$rouge,$size,'',$fonte);
	$d->pprint($copyright,10,100,$dx="",$dy="",$rouge,$size,'L',$angle);
/*
*/
