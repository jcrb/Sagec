...
$backPathToRoot = "../";
require($backPathToRoot."pma_connexion.php");// paramètres privés de connexion
...
TblDebut(0,"25%");
	TblDebutLigne();
		...
		{
			TblCellule("<a href = \"../vecteur_saisie.php?org_type=$org_type&no_liste_org='1'&back=$back\">Ajouter un véhicule</a><br>");
		}
TblFin();
...
if($_SESSION["member_id"])
{
	...
	print("<p>Moyens pouvant être mis à la disposition du SAMU</p>");
	TblDebut(0,"100%");
	$_style = "A2";
	...
	while($moyen = LigneSuivante($result))
	{
		...
		TblDebutLigne("$_style");
			...
			if($_SESSION['autorisation']==2  || $_SESSION["autorisation"]==10)
			{
				...
				TblCellule("<a href = \"apa_supprime.php?ttvecteur=$moyen->Vec_ID\" onclick=\"return confirm('Etes vous sûre de vouloir supprimer ce vecteur ?');\">Supprimer</a>");
			}
		TblFinLigne();
	}
	TblFin();
