...
//					$type = 0: tout le contenu de la table
					$type = 1: uniquement les médicaments
					$type = 2: uniquement le matériel
					$tri colonne sur laquelle se fait le tri; défaut nom spécialité
					$ordre sens du tri
//=======================================================================================*/
function liste_med($connexion,$type = 1,$tri="",$ordre="DESC")
{
	...
	$requete="SELECT medic_ID,special_nom, dci_nom,presentation_nom,presentation_ID,medic_volume,unite_abrev ml,medic_dosage,medic_dosage_unite
			FROM med_specialite,medicament LEFT OUTER JOIN (med_dci,med_presentation,med_unites)
				ON (dci_ID = medic_dci AND presentation_ID = medic_presentation AND unite_ID = medic_volume_unite)
	...
	print("<table>");
		print("<tr bgcolor=\"yellow\">");
			...
			print("<td><a href=\"?tri=2&ordre=$ordre\"><b>Spécialité</b></a></td>");
			print("<td><a href=\"?tri=3&ordre=$ordre\"><b>DCI</b></a></td>");
			print("<td><b>Présentation</b></td>");
		print("</tr>");
	while($rub=mysql_fetch_array($resultat))
	{
		print("<TR>");
...
function liste_lot($connexion,$tri="")
{
	...
	print("<TR>");
		print("<TD>ref.</TD>");
		print("<Td><a href=\"medicament_liste.php?tri=medicament\"> médicament</a></Td>");
		print("<TD>quantité</TD>");
		print("<TD><a href=\"medicament_liste.php?tri=date\">péremption</a></TD>");
