	...
	function check(n,noListe)
	{
		...
		for(var i=0; i<cases.length; i++)
		{
			if(cases[i].type == 'checkbox' && cases[i].value == n)
			{
				if(cases[i].checked)
				{
					objet_hxr.open("get","voir_hopital_enregistre.php?action=insert&value="+n+"&noListe="+noListe,true);
				}
				else
				{
					objet_hxr.open("get","voir_hopital_enregistre.php?action=del&value="+n+"&noListe="+noListe,true);
				}
...
<form name="liste">
<div id="entete">
	<p><b>Wählen Sie die Krankenhäuser zu sehen</b></p>
</div>
...
while($rub=mysql_fetch_array($resultat))
{
	...
	print("<tr>");
		print("<td align=\"left\"><input type=\"checkbox\" name=\"ch[]\" value=\"$rub[Hop_ID]\" $check onClick=\"check(".$rub[Hop_ID].",".$listeID.")\">");
