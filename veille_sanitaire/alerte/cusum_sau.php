...
/**--------------------------------------------------------------
* écriture de la requête: version expérimentale; ne concerne que 
* les services d'urgence des HUS à l'exclusion de la pédiatrie
* SAU Pasteur = 8
* SAU adulte HTP = 9
* SAU chir A = 38
-----------------------------------------------------------------*/
...
* service et un jour donné. Il existe donc plusieurs lignes ayant 
* la même date. On opère un regroupement sur les dates.
* Les données sont retournées sous forme de 3 valeurs correspondant
* aux 3 classe de mesures: <1 an, > 75 ans, entre 1 et 75 ans
* @TODO toutes les lignes de la table sont retournées alors que 
* seules les lignes correspondant au date de mesure sont utiles
* REMARQUE: il y a des jours avec données incomplètes
-----------------------------------------------------------------*/
$requete = "SELECT ";
switch($indicateur)
{
...
/*--------------------------------------------------------------------------------
* lecture des données
* le tableau srub est transformé en tableau $data comportant n lignes et 2 colonnes
...
/*----------------------------------------------------------------------------------
* Calcul du CUSUM
* calcul de la première moyenne à partir de la période de référence
* la période de référence correspond à la durée du lissage
* les données du tableau data correspondant à cette période sont saisis dans un 
* objet statistique CStat qui retourne la moyenne et l'écart-type pour cette période.
----------------------------------------------------------------------------------*/
$debut = 0;
$fin = $debut + $lissage;
...
// fichier oiur enregistrer les données et les exporter
$f=fopen('cusum.txt',"w");
...
print("<table border = \"1\" cellpadding=\"2\" cellspacing=\"0\">");
	print("<tr bgcolor=\"yellow\">");
		...
		print("<td align=\"rigth\">écart-type</td>");
...
{
	$x = ($data[$i]['passages']-$mean)/$sd - $h; // calcul du résidu t
	$s[$i] = max(0,$s[$i-1]+$x); // calcul du Cusum t
	
	// si le cusum dépasse la limite, les cases sont mise en rouge
	if($s[$i] > $limite)
	{
		$bgc='red';
		$stat_arl->addx($arl);
	...
	// @TODO voir si un reset de l'objet CStat ne serait pas plus efficient que la création nouvel objet
	//delete($Stat);
	$Stat = new CStat();
	// on incrémente
	$debut++;
