...
function ajouteScriptSagec($chpHeure, $chpMinute, $chpJourMois, $chpJourSemaine, $chpMois, $chpCommande, $chpCommentaire)
{
	...
	$isSection = false; 
	$enregistre = false;/* pour ne pas enregister à chaque fois l'en tête*/
	$regle_existe = false;
	...
	print($maxNb."<br>");
	if($maxNb == 0) // la section Sagec n'existe pas, on la crée avec la 1ère règle
	{
		$id = 1;
		//$newCrontab[] = '\n';
		$newCrontab[] = $debut;
	...
	$fp = fopen('tmp', 'w'); /* on crée le fichier temporaire */
...
function retireScript($id)
{
	...
	if(!$f = fopen("tmp2", "w")) /* on crée le fichier temporaire */
...
function affiche_crontable()
{
	...
	exec('crontab -l', $currentCrontab);		/* on récupère l'ancienne crontab dans $oldCrontab */

	foreach($currentCrontab as $ligne)			/* copie $oldCrontab dans $newCrontab sans le script à effacer */
	{
		print($ligne."<br>");
	}
}
