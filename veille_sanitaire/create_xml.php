...
//----------------------------------------- SAGEC --------------------------------------------------------
//
//	programme: 		create_xml.php
//	date de création: 	07/05/2005
//	auteur:			jcb
//	description:		crée un fichier XML selon les specifications de l'INVS
//	version:		1.0
//	maj le:			09/06/2006
//
/**
 ...
 * @version 1.0 $Id: create_xml.php 22 2007-03-12 17:13:10Z jcb $
 * @package Sagec
 */
//--------------------------------------------------------------------------------------------------------
session_start();
...
//liste_des_cles('public');
//liste_des_cles('secret');
//===================================  cryptage  ======================================================
require("gnugpg/trousseau.php");
print("  - cryptage des données<br>");
crypte($nom_fichier,MA_CLEF,SA_CLEF,PASS);
//===================================  Envoi FTP  =====================================================
/**
* Envoyer un fichier vers un serveur ftp
*
* @var $nom_fichier nom d'accès du fichier à transférer
* @return true en cas de succès de la transmission
*/
function envoi_ftp($nom_fichier,$site_ftp,$username,$password)
{
	global $envoi_reussi;
	$fichier_gpg = $nom_fichier.".gpg";
	// création d'un objet ftp
	$ftp = new sagec_ftp($site_ftp,$username,$password);
	$ftp->set_tmax(120);
	$ftp->connexion();
	print($ftp->error."<br>");
	if($ftp->error == "Connection impossible")
		return false;
	// IMPORTANT!!! turn passive mode on
	$ftp->mode_passif(true);
	$ftp->put($fichier_gpg,$fichier_gpg);
	if($ftp->error == "Echec du transfert")
	{
		print($ftp->error."<br>");
		$ftp->deconnexion();
		print($ftp->error."<br>");
	...
	print($ftp->error."<br>");
	$ftp->rename($fichier_gpg,"./RECU/".$fichier_gpg);
	print($ftp->error."<br>");//Echec du renommage
	$ftp->deconnexion();
	print($ftp->error."<br>");

	unlink($fichier_gpg);
	unlink($nom_fichier);
	print("<p>Les fichiers ".$nom_fichier." et ".$fichier_gpg." ont été effacés</p>");
	$envoi_reussi = true;
	return true;
}
...
// paramètres d'envoi
/*
$site_ftp = $crondata['cron_adresse'];
$username = $crondata['cron_login'];
$password = $crondata['cron_password'];
*/
// pour INVS
$site_ftp = "HEIMDALL.INVS.sante.fr";
//$username = "";
//$password = "I$t3st14";
$username = "Res-ARH";
$password = "Ar1h$2pr";
while($envoi_reussi != true && $nb_tentatives < $nb_max_tentatives)
{
	$nb_tentatives++;
	print("  - envoi FTP n° ".$nb_tentatives."<br>");
	$envoi_reussi = envoi_ftp($nom_fichier,$site_ftp,$username,$password);
}
if($envoi_reussi == false)
{
	print("<h1><p>ECHEC DE LA TRANSMISSION</p></h1>");
	print("<p>Les fichiers ".$nom_fichier." et ".$fichier_gpg." ont été archivés dans le répertoire</p>");
	mail_info(false);
}
else
{
	print("<h1><p>SUCCES DE LA TRANSMISSION</p></h1>");
	mail_info(true);
}
/*
// envoi ftp
$fichier_gpg = $nom_fichier.".gpg";
//send_file($fichier_gpg,$fichier_gpg);
