...
require($prefix."utilitaires/table.php");
require($prefix."utilitairesHTML.php");
require($prefix."pma_connect.php");
require($prefix."pma_connexion.php");
require($prefix."pma_requete.php");
require($prefix."en_tete.php");
require $prefix.'utilitaires/globals_string_lang.php';
require($prefix."date.php");
...
// rechercher les victimes dans les différentes zones
...
while($rub= mysql_fetch_array($resultat))
{
	...
	//	print("<td>$rub[ts_nom]</td>");
		// ventilation des victimes par gravité
		switch($rub['gravite'])
		{
			case 3:case 4:$victime[$rub['ts_nom']][1]++;break; // indéterminés, UP, UD
...
//print("</table>");
print("victimes");print_r($victime);
...
print("<table CELLSPACING=\"2\" CELLPADDING=\"0\" border=\"0\" BGCOLOR=\"\">");
	...
	if($victime)
	{
		...
		// dernière ligne = total par colonne
		$col[0] = "&nbsp;";
		$col[7] = $total;
		print("<tr>");
		for($i=0; $i<8; $i++)
...
function ligne_entete()
{
	print("<TR>");
		...
		print("<TD class=\"th1\" ALIGN=\"RIGHT\">écart-type</TD>");
...
function ligne_resultats($resultat)
{
	while($moy = mysql_fetch_array($resultat))
	{
		...
		switch($moy[5])
		{
			case '0':$titre="indéterminé";break;
			case '1':$titre="Hommes";break;
			case '2':$titre="Femmes";break;
			case '3':$titre="indéterminé";break;
			default:$titre="total";
		}
...
print("<TABLE width=\"50%\">");
	print("<TR>");
		...
		print("<TD ALIGN=\"RIGHT\"> % cumulés </TD>");
	print("</TR>");
	for($i=0; $i<=$max;$i++)
	{
		print("<TR>");
...
// ---------------------------------- histogramme des ages ---------------------------------------
print("<BR>");
print("Répartition par classes d'age<BR><BR>");
