...
$requete = "SELECT tache_ID FROM tache_cod WHERE tache_faite = 'o'";
...
{ 
	...
	$requete = "UPDATE tache_cod SET tache_faite='o',tache_heure='$date' WHERE tache_ID IN('$comma_separated')";
	$resultat = ExecRequete($requete,$connexion);
	// mise à  jour de la main courante
	$requete = "SELECT tache_message,tache_ID FROM tache_cod WHERE tache_ID IN('$comma_separated')";
	$resultat = ExecRequete($requete,$connexion);
	while($rep=mysql_fetch_array($resultat))
	{
								
		$requete = "INSERT INTO livrebord_service
	 		VALUES('','$_SESSION[organisation]','$auteur','$date','$rep[tache_message]','$groupe','o','')"; 
								
		$resultat = ExecRequete($requete,$connexion);
		$messageID = mysql_insert_id();
		$requete = "UPDATE tache_cod SET message_ID = $messageID WHERE tache_ID = $rep[tache_ID]";
...
if($decoche)
{
	...
	$requete = "UPDATE tache_cod SET tache_faite='n',tache_heure='$date' WHERE tache_ID IN('$comma_separated')";
	$resultat = ExecRequete($requete,$connexion);
	// mise Ã  jour de la main courante
	$requete = "SELECT tache_faite FROM tachecod WHERE tache_ID IN('$comma_separated')";
	$resultat = ExecRequete($requete,$connexion);
	while($rep=mysql_fetch_array($resultat))
	{
		...
		if($rep['tache_faite'])
		{
			$requete = "UPDATE livrebord_service SET LBS_visible = 'n' WHERE LBS_ID = $rep[tache_faite] AND LBS_groupe='$groupe'";
			ExecRequete($requete,$connexion);
		}
	}
	mysql_close();
